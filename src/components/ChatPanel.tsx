...
function workflowSuggestionClass(readiness: 'ready' | 'partial' | 'needs-data') {
  if (readiness === 'ready') {
    return 'border-emerald-500/25 bg-emerald-500/5 text-emerald-200 hover:bg-emerald-500/10'
  }
  if (readiness === 'partial') {
    return 'border-sky-500/25 bg-sky-500/5 text-sky-200 hover:bg-sky-500/10'
  }
...
function isImageAttachment(file: { filename?: string; mediaType?: string; url?: string }) {
    ...
    <div className="flex h-full min-h-0 flex-col bg-background">
        ...
        <ConversationContent className="gap-6">
          {runs.length === 0 ? (
            ...
            runs.map((run) => {
              ...
              const workflowCompletionSummary = businessWorkflowCompletionSummary(run, sceneAssets)
              return (
                    ...
                    <Tool key={tool.call.id} defaultOpen={tool.status === 'awaiting-approval'}>
                      ...
                      <ToolContent>
                        ...
                        <ToolOutput
                          ...
                          errorText={tool.status === 'cancelled' ? undefined : tool.error?.message}
                        />
      ...
      <div className="flex max-h-[75%] min-h-0 shrink-0 flex-col gap-2 border-t border-border p-3">
        {showInlineWorkflowTemplates && workflowEntryCards.length > 0 && (
          <>
            ...
            >
              ...
              <span className="ml-auto text-[10px]">{workflowTemplatesOpen ? '收起' : '展开'}</span>
            </Button>
                      ...
                      >
                        <div className="flex items-start gap-2">
                          <div className="min-w-0 flex-1">
                                  ...
                                  >
                                    ...
                                    <div className="min-w-0">
                                      ...
                                      {compatibleAssets.length === 0 && (
                                        <p className="mt-1 text-[9px] leading-3 text-amber-200/80">
                            ...
                            {!canStartWorkflow && (
                              <p className="mt-2 rounded-md border border-amber-500/20 bg-amber-500/5 px-2 py-1.5 text-[9px] leading-4 text-amber-100/85">
                            ...
                            <div className="mt-2 rounded-md border border-current/10 bg-background/25 px-2 py-1.5 text-[9px] opacity-80">
                              ...
                              <p className="mt-1 line-clamp-2">流程：{workflow.previewStepText}</p>
        ...
        <Suggestions>
          ...
          {primaryRecentSceneObject && (
            <Suggestion
              ...
              onClick={insertPromptText}
              className="border-purple-500/25 bg-purple-500/5 text-purple-200 hover:bg-purple-500/10"
            >
              操作最近对象
            </Suggestion>
          )}
        ...
        {inputUnavailableMessage && (
          <div
            role="status"
            className="rounded-lg border border-amber-500/20 bg-amber-500/5 px-3 py-2 text-xs leading-5 text-amber-200"
          >
            {inputUnavailableMessage}
          </div>
        )}
