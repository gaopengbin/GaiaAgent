<div align="center">
  ...
  <a href="https://github.com/gaopengbin/GaiaAgent/releases/tag/v0.3.9"><img src="https://img.shields.io/badge/release-v0.3.9-blue?style=flat-square" alt="Release v0.3.9" /></a>
...
最新版本：[GaiaAgent v0.3.9](https://github.com/gaopengbin/GaiaAgent/releases/tag/v0.3.9)
...
- **Windows 升级不再受文件锁阻塞**：内置 Node 运行时改为安装到带版本号的目录，旧版本遗留进程即使未退出，也不会阻止新安装器写入 `node.exe`。
- **更可靠的安装器清理**：NSIS 在安装和卸载前关闭 GaiaAgent 整棵进程树，并等待受管运行时进程退出。

## v0.3.8 主要更新

- **中断任务可恢复**：失败、超时、取消或断网的运行会保存原始目标、计划、工具进度、部分回答和中断原因；用户下一句只说“继续”时，也能接着完成上一项未完成任务。
- **桌面端原生资源加载**：HTTP、HTTPS、`file://`、Windows 本地路径以及嵌套的 3D Tiles/glTF 资源统一通过带临时令牌的本机代理加载，不再受 WebView 跨域限制。
- **可靠的图片粘贴发送**：输入框会直接读取剪贴板原始图片，在清空编辑器前完成附件快照，并持久化后发送给支持视觉的模型。
- **准确的工具状态**：工具结果中明确返回 `success: false`、`ok: false` 或错误状态时，现在会显示为失败，不再误标为已完成。

## v0.3.7 主要更新

- **可靠的 Windows 升级安装**：GaiaAgent 退出时关闭全部受管 MCP 子进程；NSIS 安装前只清理遗留的内置运行时进程，再安全覆盖安装文件。
- **联网任务自动收敛**：同一轮任务会去重等价的搜索与网页抓取请求，并在轮次预算耗尽前基于已有结果完成回答；内置抓取可用时不再向模型重复暴露同类外部 MCP 工具。
...
- **可靠的会话与场景绑定**：切换会话或刷新后，按确定顺序恢复底图、相机、CZML/KML/GeoJSON 图层和可重放场景内容。
- **会话级图片资产持久化**：粘贴图片获得稳定的本地附件引用，后续对话和应用重启后仍可使用，Billboard 与嵌套 CZML 图片无需公网图床。
- **更清晰的 GIS 场景树**：隐藏仅用于实现的辅助资产，用不同图标区分点、路线、面、影像、地形、模型和动画，可播放图层提供播放、暂停、复位和倍速控制。
- **更可靠的 Agent 生命周期**：失败或取消时正确结束待处理工具卡片，token 预算超限可干净停止，简单任务不再被强制生成无意义计划。
- **桌面端与运行时修复**：增强内置 GIS 工具、CC Switch/provider 路由、天地图凭据、底图持久化和 Windows 生产包启动稳定性。
